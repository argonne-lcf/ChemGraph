...
The registered workers are `single_agent`, `deep_agent`, `multi_agent`,
...
The built-in `deep_agent` entry and `main_agent`'s optional `deepagent` worker use
the shared Deep Agent constructor and default prompt supporting attached chemistry
tools. The built-in main-agent workspace worker has no chemistry tools attached.
Supply its backend through worker options when local filesystem or shell access
is intended. Agent Skills are bundled and discovered as described in
[skills](skills.md). Additional sources can be supplied alongside it with
