#!/usr/bin/env python
"""Agent + MCP + EnsembleLauncher demo running gRASPA over a CIF directory.

An LLM agent on the compute node drives a local ``graspa_mcp_hpc``
subprocess whose backend is ``ensemble_launcher``. The agent uses
``run_graspa_ensemble`` to run GCMC on every ``.cif`` in the supplied
directory at a single (T, P) condition and reports a markdown table.

Run inside ``qsub -I`` on Polaris/Aurora/Crux, in a venv where
``ensemble_launcher`` is installed. LLM API key required::

    export COMPUTE_SYSTEM=aurora
    export OPENAI_API_KEY=...
    python scripts/demo/demo_graspa_el_agent.py \\
        --model gpt-4o-mini --cif-dir /path/to/cifs --ppn 16
"""
...
async def amain(
    ...
    *,
    ...
    print(f"ppn={ppn}  ngpus_per_process={ngpus_per_process}\n")
    print("Query:\n" + "-" * 60)
    print(query)
    print("-" * 60 + "\n")
...
def main() -> None:
    ...
    parser.add_argument("--ppn", type=int, default=16,
                        help="Processes per node for the MCP backend tasks.")
    parser.add_argument("--ngpus-per-process", type=int, default=0,
                        help="GPUs per process for the MCP backend tasks.")
    ...
    asyncio.run(amain(
        args.model, system, query, args.verbose,
