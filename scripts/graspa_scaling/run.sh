...
[[ -f "${PBS_NODEFILE:-}" ]] || abort "Run with qsub, inside an Aurora PBS allocation."
SCRIPT_DIR="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)"
CG_REPO="$(cd "$SCRIPT_DIR/../.." && pwd)"
cd "$CG_REPO"
CG_REPO="$PWD"
[[ -f "$CG_REPO/scripts/graspa_scaling/run_graspa.py" ]] || abort "CG_REPO must point to the prepared PR checkout."
...
if [[ "$CG_MODEL" == alcf:* && -z "${ALCF_ACCESS_TOKEN:-}" ]]; then
    abort "Export ALCF_ACCESS_TOKEN and pass its name with qsub -v, or load it in CG_SETUP_FILE."
fi
...
cleanup() {
    trap - EXIT INT TERM
    for pid in ${CLIENT_PID:-} ${MCP_PID:-}; do
...
timeout --kill-after=30s "${CG_AGENT_TIMEOUT:-10200}" \
    "$CG_PYTHON" "$CG_REPO/scripts/graspa_scaling/run_graspa.py" \
    ...
    --model "$CG_MODEL" \
    --base-url "${CG_BASE_URL:-https://inference-api.alcf.anl.gov/resource_server/sophia/vllm/v1}" \
