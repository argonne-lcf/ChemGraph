...
def compare_llm_and_manual_workflow(manual_workflow_fp: str, llm_workflow_fp: str, print_diff=False):
    ...
    for manual_data, llm_data in zip(mdata, ldata):
        ...
        if isinstance(manual_data["manual_workflow"]["result"], str):
            ...
            if diff_final == {}:
                ...
                if print_diff:
                    print(f"Difference between manual and LLM workflow for {man_mol_smiles}")
                    print(diff_final)
        manual_set = set(
...
def main():
    result = compare_llm_and_manual_workflow(
        manual_workflow_fp="manual_workflow.json", llm_workflow_fp="llm_workflow.json", print_diff=True
    )
