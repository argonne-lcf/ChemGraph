...
def compare_llm_and_manual_workflow(
    manual_workflow_fp: str, llm_workflow_fp: str, print_diff=True, vib_task=False
):
    ...
    for key in mdata:
        if key not in ldata:
            ...
            try:
                # Handling cases when the expected return is a string
                    ...
                    if type(manual_data["manual_workflow"]["result"]) is not type(
                        ...
                        if not vib_task:
                            ...
                            if diff_final == {}:
                                ...
                                if print_diff:
                                    print(f"Difference between manual and LLM workflow for {key}: ")
                                    print(diff_final)
                        # Handling vibrational result calculation
                        else:
                            converted_manual_result = [
                                complex(val.replace("i", "j")) if "i" in val else float(val)
                                for val in manual_data["manual_workflow"]["result"]["frequency_cm1"]
                            ]
...
def main(manual_workflow, llm_workflow, print_diff, vib_task):
    result = compare_llm_and_manual_workflow(manual_workflow, llm_workflow, print_diff, vib_task)
...
if __name__ == "__main__":
    ...
    main(args.manual_workflow, args.llm_workflow, args.print_diff, args.vib_task)
