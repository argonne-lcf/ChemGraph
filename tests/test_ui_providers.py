...
def _config(argo_user=""):
    return {"api": {"openai": {"argo_user": argo_user}}}
...
def test_provider_for_model_maps_prefixes_and_lists():
    ...
    # Unknown names default to OpenAI-compatible routing.
    assert providers.provider_for_model("mystery-model").id == providers.OPENAI
...
def test_align_base_url_for_provider_repoints_shared_section():
    ...
    assert config["api"]["openai"]["base_url"] == ARGO_DEFAULT_BASE_URL
    ...
    # Other providers leave the shared section alone.
