...
def ase_parameter_agent(state: MultiAgentState, llm: ChatOpenAI):
    ...
    prompt = new_ase_parameters_input_prompt.format(geometry_response=state['geometry_response'][-1].content, feedback=feedback, ase_schema=ASESchema.model_json_schema())
    print("PROMPT: ", prompt)
    messages = [
            {"role": "system", "content": prompt},
            {"role": "user", "content": f"{state['question']}"}
    ]
    structured_llm  = llm.with_structured_output(ASESchema)
    response = structured_llm.invoke(messages).model_dump_json()
...
def construct_multi_framework_graph(llm: ChatOpenAI):
    ...
    graph_builder.add_node("GeometryTool", GeometryTool(tools=[molecule_name_to_smiles, smiles_to_atomsdata, file_to_atomsdata]))
    ...
    graph_builder.add_node("RunASE", lambda state: run_ase_multi_soft(state))
    ...
    graph_builder.add_conditional_edges(
        "GeometryAgent", route_tools_geometry,
        {"tools": "GeometryTool",
