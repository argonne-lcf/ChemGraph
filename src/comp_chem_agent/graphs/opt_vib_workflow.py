...
from comp_chem_agent.models.opt_vib_models import RouterResponse, QCEngineFeedbackResponse, ASEFeedbackResponse
...
def ase_parameter_agent(state: MultiAgentState, llm: ChatOpenAI):
    ...
    parameter_prompt = ase_parameters_input_prompt.format(geometry_response=state['geometry_response'][-1].content, feedback=feedback)
    messages = [
...
def qcengine_parameter_agent(state: MultiAgentState, llm: ChatOpenAI):
    if len(state['feedback_response']) == 0:
        ...
        feedback = state['feedback_response'][-1].content
    prompt = qcengine_parameter_prompt.format(geometry_response=state['geometry_response'][-1], feedback=feedback)
    messages = [
            {"role": "system", "content": prompt},
            {"role": "user", "content": f"{state['question']}"}
    ]
...
def end_agent(state: MultiAgentState, llm):
    prompt = end_prompt.format(output=state['opt_response'][-1].content, feedback=state['feedback_response'][-1].content)
    messages = [
        {"role": "system", "content": prompt},
        {"role": "user", "content": f"{state['question']}"}]
    response = llm.invoke(messages)
...
def construct_opt_vib_graph(llm: ChatOpenAI):
    ...
    graph_builder.add_node("ASEGeometryAgent", lambda state: ase_geometry_agent(state, llm))
    graph_builder.add_node("QCEngineGeometryAgent", lambda state: qcengine_geometry_agent(state, llm))

    graph_builder.add_node("ASEGeometryTool", ASEGeometryTool(tools=[molecule_name_to_smiles, smiles_to_atomsdata, file_to_atomsdata]))
    graph_builder.add_node("QCEngineGeometryTool", QCEngineGeometryTool(tools=[molecule_name_to_smiles, smiles_to_atomsdata, file_to_atomsdata]))

    graph_builder.add_node("ASEParameterAgent", lambda state: ase_parameter_agent(state, llm))
    graph_builder.add_node("QCEngineParameterAgent", lambda state: qcengine_parameter_agent(state, llm))
    ...
    graph_builder.add_node("QCEngineFeedbackAgent", lambda state: qcengine_feedback_agent(state, llm))
    ...
    graph_builder.add_conditional_edges(
        "RouterAgent", route_first_router, 
    ...
    graph_builder.add_conditional_edges(
        "ASEGeometryAgent", route_tools_geometry,
        {"tools": "ASEGeometryTool", "next": "ASEParameterAgent"})
    graph_builder.add_conditional_edges(
        "QCEngineGeometryAgent", route_tools_geometry,
    ...
    graph_builder.add_conditional_edges(
        "ASEFeedbackAgent",
        route_feedback,
        {"ASEParameterAgent": "ASEParameterAgent",
         "EndAgent": "EndAgent"
        })
    graph_builder.add_conditional_edges(
        "QCEngineFeedbackAgent",
        route_feedback,
        {"QCEngineParameterAgent": "QCEngineParameterAgent",
         "EndAgent": "EndAgent"
        })
