...
class SimulationInput(BaseModel):
    ...
    number_of_initialization_cycles: int = Field(default=100, alias="NumberOfInitializationCycles")
    print_every: int = Field(default=100, alias="PrintEvery")
    print_forcefield_to_output: str = Field(default="no", alias="PrintForcefieldToOutput")
    ...
    framework: int = Field(default=0, alias="Framework", description="Index of framework, starts from 0.")
    framework_name: str = Field(default=None, alias="FrameworkName")
    unit_cells: List[int] = Field(default=[1, 1, 1], alias="UnitCells", description="Framework name")
    external_temperature: float = Field(default=273, alias="ExternalTemperature", description="Temperature in Kelvin (K)")
    external_pressure: float = Field(default=1e6, alias="ExternalPressure", description="Pressure in Pascal (Pa)")
