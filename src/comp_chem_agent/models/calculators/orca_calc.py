...
class OrcaCalc(BaseModel):
    calculator_type: str = Field(
        default="orca",
        description="Calculator type. Currently supports only 'orca'."
    )
    charge: int = Field(
        default=0,
    ...
    orcablocks: str = Field(
        default="%pal nprocs 1 end",
        description="Additional ORCA block settings."
    )
    directory: str = Field(
        default=".",
        description="Working directory for ORCA calculations."
    )
    profile: Optional[str] = Field(
        default=None,
        description="Optional ORCA profile configuration."
    )

    def get_calculator(self):
        """Returns an ASE-compatible ORCA calculator instance."""
        if self.calculator_type != "orca":
            raise ValueError("Invalid calculator_type. The only valid option is 'orca'.")
