...
def _render_argo_card(draft: dict, info, status) -> None:
    ...
    current = draft["api"]["openai"].get("argo_user", "")
    user = st.text_input(
        "ANL username",
        value=current,
    ...
    if user != current:
        draft["api"]["openai"]["argo_user"] = user
        st.session_state.config["api"]["openai"]["argo_user"] = user
        save_config(st.session_state.config)
        st.rerun()
    st.caption(status.detail)
    # The gateway URL lives in the shared [api.openai] section; it is
    # edited on the OpenAI card and repointed automatically on "Use".
    st.caption(
        "Gateway: "
        f"`{draft['api']['openai'].get('base_url') or 'default Argo endpoint'}`"
    )
...
def _render_api_key_card(draft: dict, info, status) -> None:
    ...
    with col_clear:
        if key_set and st.button("Clear key", key=f"provider_clear_{info.id}"):
            ...
            st.rerun()
    # Groq is routed by the SDK default; nothing reads [api.groq], so an
    # endpoint editor there would be a silent no-op.
    if info.config_section and info.id != providers.GROQ:
