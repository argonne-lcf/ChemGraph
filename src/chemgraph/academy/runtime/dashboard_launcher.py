...
from chemgraph.academy.dashboard.server import _build_launch_config
...
def _build_launch_pieces(args):
    """Build (launch_config, launch_controller) when --enable-launch-buttons.

    Returns (None, None) when the feature is off so serve_dashboard's
    existing-behavior path is unchanged.
    """
    if not getattr(args, "enable_launch_buttons", False):
        return None, None
    # server._build_launch_config expects args.system as a comma string.
    # dashboard_launcher parses --system into a tuple, so translate.
    import argparse as _argparse
    shim = _argparse.Namespace(**vars(args))
    if isinstance(shim.system, (tuple, list)):
        shim.system = ",".join(shim.system)
    cfg = _build_launch_config(shim)
    # Empty sites -- /api/launch sets them from the canvas placement
    # at launch time.
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(prog="swarm dashboard")
    a = p.add_argument
    # run_id: the on-disk scratch bucket name shared by dashboard
    # (local mirror) and HPCs (run_dir). Historically required, now
    # optional -- if omitted we generate a timestamp so operators
    # who just want to launch a canvas-authored campaign do not have
    # to invent a name.
    #
    # Ceiling: campaign selection is canvas-time, so the ideal
    # run_id per LAUNCH is the campaign name (isolates campaigns,
    # same-name relaunch wipes). Requires restarting rsync loops on
    # every /api/launch; deferred until we hit an operator who
    # actually asks for it. For now, a timestamp gives you a clean
    # scratch dir per dashboard session which is close enough.
    a(
        "run_id",
        nargs="?",
        default=None,
        help=(
            "Optional scratch-bucket name for this dashboard session. "
            "Defaults to 'session-YYYY-MM-DD-HHMM'. Every Launch click "
            "in this session writes into <run_root>/<run_id>/."
        ),
    )
    a(
        "--system",
    ...
    # --overwrite-run was retired: run_dir is now wiped per-launch on
    # every /api/launch (server.py), so the operator can iterate on a
    # campaign in the canvas without restarting the dashboard between
    # attempts. Same-name = override, different-name = fresh scratch.
    # Launch-button feature (opt-in). When set, the dashboard exposes
    # POST /api/launch + /api/inject-message so the operator can drive the
    # launcher subprocess from the browser. See server.py.
    a("--enable-launch-buttons", action="store_true")
    a("--bundle-root", help="Forwarded to launcher when --enable-launch-buttons.")
    a("--project", help="Forwarded to launcher as --project.")
    args = p.parse_args()
...
def compute_lines(profile: SystemProfile, wrapper_path: str, run_id: str) -> list[str]:
    ...
    return lines + [f"  source {profile.remote_root}/venvs/academy-swarm/bin/activate", f"  export PATH={profile.remote_root}/bin:$PATH", "  swarm run-compute \\", f"    --system {profile.name} \\", f"    --run-id {run_id} \\", "    --campaign <name>", "", "If PATH is not configured, use:", f"  {wrapper_path} \\", f"    --system {profile.name} \\", f"    --run-id {run_id} \\", "    --campaign <name>"]
...
def _setup_site(
    *,
    ...
    # Default to the same ControlPath the operator's ~/.ssh/config
    # uses (~/.ssh/cm-%r@%h:%p per the project's ALCF auth setup) so
    # the dashboard's ssh tunnels share the master that rsync, the
    # launcher, and the operator's interactive shells already opened
    # -- one MFA prompt per HPC per 4h instead of one per dashboard
    # start. The previous per-system pattern (.ssh/{system}-dashboard-...)
    # opened a SEPARATE master that re-authed every time.
    #
    # If the operator's config uses a different ControlPath pattern
    # the dashboard will silently miss the existing master and prompt
    # for password -- same behavior as before, no regression.
    control_path = (
        args.ssh_control_path
        or str(Path.home() / ".ssh/cm-%r@%h:%p")
    )
    ...
    wrapper_path = f"{profile.remote_root}/bin/swarm-run"
    ...
    # selected via the canvas + POST /api/launch, not a dashboard-
    # startup arg, so it is intentionally absent from this payload.
    metadata: dict[str, Any] = {
        "created_at": time.time(),
        "created_by": "swarm dashboard",
        "run_id": args.run_id,
        "system": profile.name,
    ...
    # Per-site filename: run_dir is shared across sites in federated
    # runs (Eagle mount), so a single dashboard_metadata.json would
    # race -- last writer wins and gives every site (including the
    # other one's) compute_launcher whichever lm_base_url landed last.
    # Also write the legacy un-suffixed name so single-site reads keep
    # working when only one site exists.
    meta_name = f"dashboard_metadata.{profile.name}.json"
    print(f"[{profile.name}] Writing run metadata: {remote_host}:{remote_run_dir}/{meta_name}", flush=True)
    ssh(
        remote_host,
        f"mkdir -p {shlex.quote(remote_run_dir)} && "
        f"cat > {shlex.quote(remote_run_dir + '/' + meta_name)}",
        control_path=control_path,
        input_text=json.dumps(metadata, indent=2) + "\n",
    )
    ...
    print(f"\n[{profile.name}] Compute-node command (choose --campaign in the canvas):", flush=True)
    print("\n".join(compute_lines(profile, wrapper_path, args.run_id)), flush=True)
...
def main() -> int:
    args = parse_args()
    # Auto-name the session if the operator did not pass a positional.
    # Compact timestamp: session-YYYY-MM-DD-HHMM, sortable + unique
    # per-minute (a re-launch inside the same minute reuses the bucket,
    # which is fine -- per-launch wipe handles the collision).
    if not args.run_id:
        args.run_id = time.strftime("session-%Y-%m-%d-%H%M")
        print(f"[dashboard] auto-generated run_id: {args.run_id}", flush=True)
    ...
    if args.local:
        ...
        launch_cfg, launch_ctrl = _build_launch_pieces(args)
