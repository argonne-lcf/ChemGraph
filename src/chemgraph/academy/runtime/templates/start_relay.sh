...
find_orphan_pids() {
  ...
  # Match by argv BASENAME of the relay script, not the full path.
  # Eagle is mounted as both /eagle/... and /lus/eagle/... on ALCF UANs;
  # a prior session that recorded the /lus/... form leaves an orphan
  # whose argv exact-string-mismatches the new session's /eagle/... form
  # (or vice versa) -- the orphan survives "cleanup" and the bind fails.
  local rs_base
  rs_base="$(basename "${RELAY_SCRIPT}")"
  ps -u "${USER}" -o pid=,comm=,args= 2>/dev/null \
    | awk -v rb="${rs_base}" -v me="${self_pid}" -v pp="${parent_pid}" '
        $1 == me || $1 == pp { next }
        $2 ~ /python/ {
          for (i = 3; i <= NF; i++) {
            n = split($i, parts, "/")
            if (parts[n] == rb) { print $1; next }
          }
        }
      '
}
...
if command -v ss >/dev/null 2>&1; then
  if ss -tln 2>/dev/null | awk '{print $4}' | grep -qE "[:.]${RELAY_PORT}\$"; then
    # Fallback: argv-basename match missed it (e.g. process started from
    # a different python binary, or argv got rewritten). Kill by port
    # ownership instead. Still scoped to OUR uid because ss -tlnp only
    # shows other users' pids when root.
    port_pids="$(ss -tlnp 2>/dev/null \
      | awk -v p="${RELAY_PORT}" '$4 ~ "[:.]"p"$" { print }' \
      | grep -oE 'pid=[0-9]+' | cut -d= -f2 | sort -u)"
    for pid in ${port_pids}; do
      [ "${pid}" = "$$" ] && continue
      [ "${pid}" = "${PPID:-0}" ] && continue
      echo "[start_relay] killing port-holder pid=${pid} on port ${RELAY_PORT}"
