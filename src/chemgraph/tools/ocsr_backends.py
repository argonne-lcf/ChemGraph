...
def smiles_from_llm(image_bytes: bytes, mime: str, backend: str,
                    model: str | None = None, timeout_s: float = 90.0,
                    prompt_mode: str = "legacy_benchmark") -> dict:
    ...
    prompt_mode
        ``"legacy_benchmark"``, the default, sends the vendored prompt behind every
        number in the shipped calibration table. ``"structured"`` asks for a JSON
        object with a ``smiles`` field, which removes the guesswork from parsing at
        the cost of a prompt the shipped table was not fitted under; refit with
        ``ocsr_calibrate`` to get confidence numbers that describe it.
    """
    import base64

    start = time.monotonic()
    if prompt_mode not in ("legacy_benchmark", "structured"):
        return _narrow(error=f"unknown prompt_mode {prompt_mode!r}. "
                             f"Choose 'legacy_benchmark' or 'structured'.")
    if backend == "llm":
    ...
    try:
        ...
        system = (ocsr_prompt.OCSR_STRUCTURED_SYSTEM_PROMPT
                  if prompt_mode == "structured" else ocsr_prompt.OCSR_SYSTEM_PROMPT)
