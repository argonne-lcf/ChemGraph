...
def run_graspa_parsl_app(job: dict):
    ...
    """
    from chemgraph.tools.graspa_tools import graspa_input_schema, run_graspa_core
...
@mcp.tool(
    ...
    description="Run a single graspa calculation",
)
def run_mace_single(graspa_input_schema: graspa_input_schema):
    return run_graspa_core(graspa_input_schema)


@mcp.tool(
    name="run_graspa_ensemble",
    description="Run an ensemble of graspa calculations",
)
async def run_graspa_ensemble(
    ...
    dict
        {
          ...
          "results": [
            {
              "structure": "<filename>",
              "result": <whatever run_graspa_core returns>,  # e.g. (uptake, error)
            }
            or, on error:
            {
              "structure": "<filename>",
    ...
    """
    input_dir = Path(graspa_input_schema_ensemble.input_structure_directory)
    ...
    # Collect all .cif files in the directory
    ...
    base_output = Path(graspa_input_schema_ensemble.output_result_file)
    base_stem = base_output.stem
    base_suffix = base_output.suffix or ".json"

    pending_tasks = []
    for struct_path in structure_files:
        # Per-structure output file (if your real run_graspa_core uses it)
        ...
        job = {
            ...
            "temperature": graspa_input_schema_ensemble.temperature,
            "pressure": graspa_input_schema_ensemble.pressure,
            "adsorbates": graspa_input_schema_ensemble.adsorbates,
            "adsorbate_compositions": graspa_input_schema_ensemble.adsorbate_compositions,
        }

        fut = run_graspa_parsl_app(job)
        # futures.append((struct_path.name, fut))
        pending_tasks.append((struct_path.name, fut))
