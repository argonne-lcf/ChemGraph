...
somewhere. If you have none, `--validate` against the shipped table is the cheaper
question, since detecting "this table is wrong for my images" takes far fewer labels
than fitting a new one.
"""
...
def _interval(k: int, n: int, alpha: float = 0.05) -> list[float]:
    """Jeffreys 95% credible interval, or a Wilson fallback without scipy."""
    try:
...
def collect(labels: list[tuple[str, str]], models: list[str],
    ...
    for i, (image_path, reference) in enumerate(labels, 1):
        try:
            image_bytes, _ = core.load_image_bytes(image_path)
...
def fit_calibration(rows: list[tuple[list[dict], str]], models: list[str],
    ...
    for pattern, (k, n) in sorted(buckets.items(), key=lambda kv: -kv[1][1]):
        cell: dict = {"k": k, "n": n, "ci": _interval(k, n)}
        if n >= min_n:
...
def validate(rows: list[tuple[list[dict], str]], models: list[str],
    ...
    if not judged:
        ...
        lines.append(f"Largest gap {100*worst:.0f} pp. The shipped table looks usable "
                     f"for your images.")
    else:
        lines.append(f"Largest gap {100*worst:.0f} pp. The shipped table does NOT "
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument("--validate", action="store_true",
                   help="check the shipped table against your images instead of fitting")
    ...
    if missing:
        print(f"not installed: {', '.join(missing)}. Installed: "
              ...
              f"Install with: python -m chemgraph.tools.ocsr_setup <model>",
              file=sys.stderr)
        return 2
    if not models:
    ...
    print(f"running {len(models)} model(s) over {len(labels)} images; the first call "
          f"per model loads it (5-66 s)", file=sys.stderr)
    ...
    if args.validate:
        print(validate(rows, models, core.load_calibration()))
        return 0
    ...
    if args.out:
        ...
        print(f"\nwrote {args.out}\nUse it with: "
              f"image_to_smiles_core(img, backend='ensemble', {wanted}"
