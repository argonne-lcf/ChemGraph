...
def image_to_smiles_core(image_path: str, backend: str = "auto",
                         ...
                         report_solo_accuracy: bool = False,
    ...
    prompt_mode : str, optional
        For a vision LLM backend. ``"legacy_benchmark"``, the default, sends the
        vendored prompt behind every number in the shipped calibration table.
        ``"structured"`` asks for a JSON object with a ``smiles`` field, so the answer
        arrives in a named field; refit with ``ocsr_calibrate`` for confidence numbers
        that describe a structured run.
