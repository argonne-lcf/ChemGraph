"""Backend-agnostic general ASE MCP server.
...
This mirrors :mod:`chemgraph.mcp.mace_mcp_hpc` but exposes the full ASE
surface (selectable calculator: EMT, TBLite/xTB, MACE, ... -- whatever is
installed) rather than hardwiring MACE. The tool bodies delegate straight
to :func:`chemgraph.tools.ase_core.run_ase_core`; no MACE->ASE adapter is
needed because the schema is already ASE-native.
...
mcp = CGFastMCP(
    ...
    instructions="""
        You expose tools for running ASE simulations and reading their results.
        The available tools are:
        1. run_ase_single: run a single ASE calculation.
...
def _ase_worker(job: dict) -> dict:
    """Execute a single ASE simulation on a backend worker.
    ...
    if remote_file is not None:
        ...
        if not os.path.isabs(job.get("output_results_file", "")):
            job["output_results_file"] = os.path.join(
                os.path.dirname(remote_file),
                job.get("output_results_file", "output.json"),
            )
    ...
    if inline is not None:
        ...
        tmpdir = tempfile.mkdtemp(prefix="chemgraph_ase_")
        xyz_path = os.path.join(tmpdir, "structure.xyz")
        ase_write(xyz_path, atoms)
        job["input_structure_file"] = xyz_path
        if not os.path.isabs(job.get("output_results_file", "")):
            job["output_results_file"] = os.path.join(
                tmpdir, job.get("output_results_file", "output.json")
            )

    output_file = job.get("output_results_file")
    if output_file:
...
# (this file is run as ``python -m chemgraph.mcp.ase_mcp_hpc``) and falls
# back to pickle-by-value, which walks the module's globals and tries to
# serialize the dynamic ``run_ase_singleArguments`` class held by
...
CGFastMCP._fix_module_for_pickle(_ase_worker)
...
def _ase_transport_hook(task: TaskSpec) -> TaskSpec:
    ...
    logger.debug(
        "ase transport hook: task_id=%s callable=%s",
        task.task_id,
        getattr(task.callable, "__qualname__", task.callable),
    )
    if task.callable is run_ase_single:
...
mcp.set_pre_submit_hook(_ase_transport_hook)


# ── Single-structure tool ──────────────────────────────────────────────


def run_ase_single(params: ASEInputSchema) -> dict:
    """Run a single ASE calculation on the configured backend.

    The pre-submit hook rewrites this call to invoke ``_ase_worker``
    ...
    # same worker.
    return _ase_worker(params.model_dump())
...
def _expand_ase_ensemble(params: ase_input_schema_ensemble) -> list[dict]:
    ...
    """
    # Keep the calculator as a plain dict so job dicts pickle cleanly to
    # remote workers (dynamic calculator models can trip cloudpickle).
    calculator = params.calculator
    if hasattr(calculator, "model_dump"):
        calculator = calculator.model_dump()
...
def run_ase_ensemble(params: ase_input_schema_ensemble) -> list[dict]:
    return _expand_ase_ensemble(params)
...
if __name__ == "__main__":
    ...
    mcp.tool(
        name="run_ase_single",
        description="Run a single ASE calculation",
        processes_per_node=_args.ppn,
        gpus_per_task=_args.ngpus_per_process,
    )(run_ase_single)

    mcp.schema_fanout_tool(
        name="run_ase_ensemble",
        description=(
            "Run ASE calculations over every structure in a directory. "
