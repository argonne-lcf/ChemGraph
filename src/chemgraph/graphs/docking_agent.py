...
def route_tools(state: State):
    """Route to the 'tools' node if the last message has tool calls; otherwise 'done'.
...
def ResponseAgent(state: State, llm: ChatOpenAI, formatter_prompt: str):
    """An LLM agent responsible for formatting the final message.
...
def construct_docking_graph(
    ...
    """
    try:
        logger.info("Constructing docking graph")
        ...
        if not structured_output:
            ...
            logger.info("Docking graph construction completed")
            return graph
        else:
            graph_builder.add_node(
                "ChemGraphAgent",
