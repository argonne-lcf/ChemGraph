...
def ReportAgent(state: State, llm: ChatOpenAI, system_prompt: str, tools=None):
    """LLM node that processes messages and decides next actions.
...
def construct_single_agent_report_graph(
    llm: ChatOpenAI,
    ...
    report_prompt : str, optional
        The report prompt to guide the LLM's behavior, by default report_prompt
    tool: list, optional
    ...
    try:
        ...
        graph_builder.add_node("tools_report", tool_node_report)
        ...
        graph_builder.add_conditional_edges(
            "ReportAgent",
            route_tools,
            {"tools": "tools_report", "done": END},
        )
        graph_builder.add_edge("tools", "ChemGraphAgent")
        graph_builder.add_edge("tools_report", "ReportAgent")
        graph_builder.add_edge(START, "ChemGraphAgent")
        graph_builder.add_edge("ReportAgent", END)
